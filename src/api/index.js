...
const getHomeworld = async (url) => {
	try{
		...
		swal("Oops!", "Seems like we couldn't fetch the info to homeworld", "error");
	}
}

const getDetail = async (url) => {
	try{
		let data = await Promise.all(url?.map(async (film) => {
...
export {
	...
	//ESTE CODIGO LO COLOQUE EN LINEA 27, PERO FUE REEMPLAZADO PARA TENER LA INFO MAS DETALLADA Y ESTRUCTURADA
